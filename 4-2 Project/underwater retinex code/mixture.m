...
subplot(2,1,2);
imhist(rgb2gray(enhanced_weight));
title('Gray Scale Histogram of Enhanced Image');
drawnow;pause(0.2);

% Assuming you have these variables already:
% rgbImage - original RGB image
% enhanced_weight - enhanced RGB image

% Convert RGB channels to uint8 for histogram plotting
R1 = uint8(255 * rgbImage(:,:,1));
G1 = uint8(255 * rgbImage(:,:,2));
B1 = uint8(255 * rgbImage(:,:,3));

R2 = uint8(255 * enhanced_weight(:,:,1));
G2 = uint8(255 * enhanced_weight(:,:,2));
B2 = uint8(255 * enhanced_weight(:,:,3));
